...
(function($) {
	...
	var weebox = function(content,options) {
		...
		this.initBox = function() {
			...
			switch(self.options.type) {
				...
				html =  '<div class="webox_base">' + 
						...
						'		</div>' +
						'	</div>' +
						'	<a href="javascript:;" onclick="return false" title="关闭" class="dialog-close"></a>' +
						'</div>';
						break;
				case 'custom':
				case 'suggest':
			...
			if (self.options.positionFix) {
				self.dh.addClass('webox-fix');
			}	
		...
		this.initContent = function(content) {
			self.dh.find(".dialog-ok").text(self.options.okBtnName);
			self.dh.find(".dialog-cancel").text(self.options.cancelBtnName);	
			if(self.options.boxclass=='' || self.options.boxclass=='weedialog'){
					self.options.showButton=false;
			}
			if(self.options.boxclass=='webox_custom'){
					self.options.showButton=false;
					self.options.showClose=true;
					self.options.modal=true;
			}
			if (self.options.title == '') {
				if(self.options.boxclass=='webox_tips' || self.options.boxclass=='webox_tipsOk'){
					self.dt.html('提示');
				}
				//self.dt.hide();	
		...
		this.show = function() {
			if (self.options.showClose) {
				self.dh.find('.dialog-close').show();
			}	
			if (self.options.showButton) {
	...
	var webox = function() {		
		...
		//关闭最上层窗体,程序调用方法：jQuery.webox.close();
		this.close = function(){
